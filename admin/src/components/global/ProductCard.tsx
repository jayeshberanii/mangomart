...
const ProductCard = ({ product }: { product: ProductType }) => {
    return (
        <div className="group flex w-[240px] max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md">
            <a className="relative mx-3 mt-3 flex h-40 overflow-hidden rounded-xl" href="#">
                <LazyImage
                    className="peer absolute top-0 right-0 h-full w-full object-cover"
                    src={product.imageLink} alt="product image"
                    onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
                        const target = e.currentTarget;
                        target.onerror = null;
                        target.src = "/product-fallback.png";
                    }}
                />
            ...
            <div className="mt-4 px-5 pb-5 flex-1 flex gap-6 flex-col justify-between">
                <a href="#">
                    <h5 className="text-lg leading-6 text-slate-900">{product?.name?.length > 30 ? product?.name?.slice(0, 30) + "..." : product?.name}</h5>
                </a>
                <Link to={`/product/${product?._id}`} className="flex mt-auto items-center justify-center rounded-md bg-slate-900 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-blue-300">
                    <svg xmlns="http://www.w3.org/2000/svg" className="mr-2 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                    </svg>
                    {product?.mrp?.currency} {product?.mrp?.amount}
                </Link>
            </div>
        </div>
